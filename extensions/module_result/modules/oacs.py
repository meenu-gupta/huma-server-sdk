...
from extensions.module_result.questionnaires.oacs_questionnaire_calculator import (
    OACSQuestionnaireCalculator,
)
from sdk.common.exceptions.exceptions import InvalidRequestException


class OACSModule(QuestionnaireModule):
    moduleId = "OACS"
    primitives = [Questionnaire, OACS]
    calculator = OACSQuestionnaireCalculator
    ragEnabled = True
    minimumAnswered = 12
    ...
    def preprocess(self, primitives: list[Primitive], user: Optional[User]):
        ...
        oacs_dict = {
            ...
            **{key: 0.0 for key in OACS.__annotations__.keys()},
        }
        oacs = OACS.from_dict(oacs_dict)
        oacs.scoring_answers = questionnaire.answers
        primitives.append(oacs)
