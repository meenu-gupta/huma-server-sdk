...
VALID_MANAGER_2_ID = "5e8f0c74b50aa9656c34788d"
...
VALID_MANAGER_1_DEPLOYMENT_2_ID = "5e8f0c74b50aa9656c34786d"
...
class BaseAssignManagerTestCase(ExtensionTestCase):
    components = [
        AuthComponent(),
    ...
    def setUp(self):
        ...
        self.assign_path = f"{self.user_path}/%s/assign"
        self.headers = self.get_headers_for_token(VALID_MANAGER_ID)


class AssignManagerTestCase(BaseAssignManagerTestCase):
    ...
    def test_success_assign_admin_to_new_user(self):
        ...
        body = {self.MANAGER_IDS_FIELD: [VALID_MANAGER_ID]}
        self.post_with_manager_headers(self.assign_path % VALID_USER_ID_1, body)
    ...
    def test_success_assign_invalid_admin_to_user(self):
        body = {self.MANAGER_IDS_FIELD: ["449213d10b6d06b35e133b79"]}
        r = self.post_with_manager_headers(self.assign_path % VALID_USER_ID_1, body)
    ...
    def test_success_assign_admin_to_old_user(self):
        ...
        self.assertEqual(VALID_MANAGER_ID, str(old_record[self.MANAGER_IDS_FIELD][0]))

        body = {self.MANAGER_IDS_FIELD: [VALID_MANAGER_ID]}
        self.post_with_manager_headers(self.assign_path % VALID_USER_ID_2, body)

        new_record = self.get_existing_assignment_record(VALID_USER_ID_2)
        self.assertEqual(VALID_MANAGER_ID, str(new_record[self.MANAGER_IDS_FIELD][0]))
    ...
    def test_success_assign_contributor_to_new_user(self):
        ...
        body = {self.MANAGER_IDS_FIELD: [CONTRIBUTOR_1_ID_DEPLOYMENT_X]}
        self.post(
            self.assign_path % VALID_USER_ID_1,
            body,
    ...
    def test_success_assign_contributor_to_old_user(self):
        ...
        body = {self.MANAGER_IDS_FIELD: [CONTRIBUTOR_1_ID_DEPLOYMENT_X]}
        self.post(
            self.assign_path % VALID_USER_ID_2,
            body,
            self.get_headers_for_token(CONTRIBUTOR_1_ID_DEPLOYMENT_X),
        )
    ...
    def test_success_assign_custom_role_with_manage_patient_data_to_new_user(self):
        ...
        body = {
            ...
            self.DEPLOYMENT_ID_FIELD: DEPLOYMENT_ID,
        }
        self.post(
            self.assign_path % VALID_USER_ID_1,
            body,
    ...
    def test_failure_assign_user_to_user(self):
        body = {self.MANAGER_IDS_FIELD: [VALID_USER_ID_1]}
        assign_path = self.assign_path % VALID_USER_ID_2
        response = self.post(assign_path, body, self.headers)
        self.assertEqual(403, response.status_code)


class RetrieveAssignedManagersTestCase(BaseAssignManagerTestCase):
    ...
    def get_user_profile(self, user_id: str, manager_id: str = None):
        response = self.flask_client.get(
            ...
            headers=self.get_headers_for_token(
                manager_id if manager_id else VALID_MANAGER_ID
            ),
        )
        if not manager_id:
    ...
    def test_success_retrieve_assigned_managers(self):
        ...
        self.assertEqual(VALID_MANAGER_ID, response_json[field_name][0])
    ...
    def _get_profiles_by_second_manager(self, body: dict):
        headers = self.get_headers_for_token(VALID_MANAGER_2_ID)
    ...
    def test_success_retrieve_managers_with_assigned_patients_count(self):
        ...
        filtered = filter(lambda user: user["id"] == VALID_MANAGER_ID, response.json)
    ...
    def test_failure_retrieve_assigned_users_for_another_manager_another_deployment(
        ...
        manager_id = VALID_MANAGER_1_DEPLOYMENT_2_ID
        response = self.flask_client.get(
            f"{self.user_path}/{manager_id}/profiles/assigned",
            query_string={"deploymentId": DEPLOYMENT_2_ID},
...
class DeleteUserManagerTest(BaseAssignManagerTestCase):
    MANAGER_IDS_FIELD = "managerIds"
    ...
    def test_success_delete_user_is_patient(self):
        ...
        body = {self.MANAGER_IDS_FIELD: [VALID_MANAGER_ID]}
        self.post(self.assign_path % VALID_USER_ID_1, body)
        new_record = self.get_existing_assignment_record(VALID_USER_ID_1)
        self.assertIsNotNone(new_record)
    ...
    def test_success_delete_user_is_manager(self):
        ...
        body = {self.MANAGER_IDS_FIELD: [VALID_MANAGER_2_ID, VALID_MANAGER_ID]}
        self.post(self.assign_path % VALID_USER_ID_1, body)
        new_record = self.get_existing_assignment_record(VALID_USER_ID_1)
        self.assertIsNotNone(new_record)

        super_admin_headers = self.get_headers_for_token(VALID_SUPER_ADMIN_ID)
        del_user_path = f"/api/extensions/v1beta/user/{VALID_MANAGER_2_ID}/delete-user"
        rsp = self.flask_client.delete(del_user_path, headers=super_admin_headers)
        self.assertEqual(rsp.status_code, 204)

        res = self.get_existing_assignment_record(VALID_USER_ID_1)
        managers_in_res = [str(i) for i in res["managerIds"]]
        self.assertNotIn(VALID_MANAGER_2_ID, managers_in_res)
