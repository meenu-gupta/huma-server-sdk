...
class UserNotesV1TestCase(IntegrationTestCase):
    components = [
        AuthComponent(),
    ...
    def setUpClass(cls) -> None:
        super(UserNotesV1TestCase, cls).setUpClass()
        cls.deployment_id = "5d386cc6ff885918d96edb2c"
        cls.user_id = "5e8f0c74b50aa9656c34789c"
        cls.admin_id = "5e8f0c74b50aa9656c34789d"
        cls.base_path = "/api/extensions/v1/user"

    def _send_add_user_notes_request(self, user_id: str, deployment_id: str, note: str):
    ...
    def _send_retrieve_user_notes_api_request(
        ...
        headers = self.get_headers_for_token(self.admin_id)
        skip_query_param = f"{RetrieveUserNotesRequestObject.SKIP}={skip}"
        limit_query_param = f"{RetrieveUserNotesRequestObject.LIMIT}={limit}"
        return self.flask_client.get(
            f"{self.base_path}/{user_id}/deployment/{deployment_id}/notes?{skip_query_param}&{limit_query_param}",
            headers=headers,
        )
    ...
    def test_add_user_note_v1(self):
        rsp = self._send_add_user_notes_request(
            self.user_id, self.deployment_id, "note"
        )
        self.assertEqual(201, rsp.status_code)

    def test_retrieve_user_note_v1(self):
        sample_note = "test note"
        rsp = self._send_add_user_notes_request(
            self.user_id, self.deployment_id, sample_note
        )
        self.assertEqual(201, rsp.status_code)
        rsp = self._send_retrieve_user_notes_api_request(
            self.user_id, self.deployment_id
        ...
        note_texts = [user_note.get(UserNote.NOTE, "") for user_note in rsp.json]
        self.assertIn(sample_note, note_texts)
