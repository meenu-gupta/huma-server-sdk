...
class Migration(BaseMigration):
    def upgrade(self):
        ...
        for profile in export_profiles:
            ...
            deidentify_fields.append(User.FAMILY_MEDICAL_HISTORY)
            deidentify_fields.append(User.EXTRA_CUSTOM_FIELDS)
            deidentify_fields[:] = list(set(deidentify_fields))
            if User.DATE_OF_BIRTH in deidentify_fields:
                deidentify_fields.remove(User.DATE_OF_BIRTH)
